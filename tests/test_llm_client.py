...
def test_openai_client(mock_openai, mock_async_openai):
    ...
    async def async_test():
        ...
        mock_async_openai.return_value.chat.completions.create = AsyncMock(return_value=mock_async_stream)
...
def test_ollama_client(mock_ollama_client, mock_async_ollama_client):
    ...
    # Test chat_response
    mock_ollama_client.return_value.chat.return_value = {'message': {'content': 'Test response'}}
    response = client.chat_response('system', 'user', {'temperature': 0.7, 'top_p': 0.9})
    assert response == 'Test response'
    ...
    async def async_test():
        mock_async_stream = AsyncMock()
        mock_async_ollama_client.return_value.chat = AsyncMock(return_value=mock_async_stream)
